...
const originalRule = require('./jest-mock-path-exists')
...
originalFs.statSync = (filePath) => {
  if (mockDirs[filePath]) {
...
mockDirs[path.join(projectRoot, 'src/components/Button')] = true
mockDirs[path.join(projectRoot, 'src/components/Button.tsx')] = true
mockDirs[path.join(projectRoot, 'node_modules/react')] = true
mockDirs[path.join(projectRoot, 'src/utils')] = true
...
const tests = {
  valid: [
    ...
    {
      code: `const something = require('path/does/not/exist')`,
      filename: path.join(process.cwd(), 'src/components/Test.tsx'),
    },
  ],
  invalid: [
...
originalFs.statSync = originalStatSync
