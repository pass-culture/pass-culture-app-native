...
| **Haute**   | **Architecture & État** | 🔹 Remplacer la majorité des `Context` par des stores centralisés (**Zustand**)🔹 Utiliser **React Query** pour tout l'état serveur🔹 Établir l'**URL comme source de vérité**                       | **Maintenabilité**              |
...
| **Moyenne** | **Qualité du Code**     | 🔹 Refactorer les hooks complexes (ex: `useCtaWordingAndAction`) en suivant le découpage `Page` / `Container` / `Presentational` component🔹 Réduire la complexité cognitive des fonctions critiques | **Maintenabilité, Stabilité**   |
| **Moyenne** | **API & Requêtes**      | 🔹 Faire évoluer le backend pour qu'il retourne des données pré-formatées🔹 Mettre en place un PoC avec **Orval** pour simplifier la génération du code d'appel API                                  | **Performance, Maintenabilité** |
