...
const getDateAtGivenHour = (date: Date, hours: number): Date =>
 ...
 */
const getTimestampsFromTimeRangeAndDate = (date: Date, timeRange: Range<number>): Range<Date> =>
  timeRange.map((hour) => getDateAtGivenHour(date, hour)) as Range<Date>

const getTimestampsFromTimeRangeAndDates = (
  dates: Date[],
  timeRange: Range<number>
): Array<Range<Date>> =>
  dates.map((date: Date) => getTimestampsFromTimeRangeAndDate(date, timeRange))
...
const computeTimeRangeFromHoursToSeconds = (timeRange: Range<number>) => {
