...
describe('TIME', () => {
  describe('utils', () => {
    it('should getStartOfDay', () => {
      expect(TIME.getStartOfDay(new Date(2021, 4, 3, 9, 30))).toEqual(new Date(2021, 4, 3))
    })

    it('should getEndOfDay', () => {
      expect(TIME.getEndOfDay(new Date(2021, 4, 3, 9, 30))).toEqual(
        new Date(2021, 4, 3, 23, 59, 59)
      )
    })

    it('should getDateAtGivenHour', () => {
      expect(TIME.getDateAtGivenHour(new Date(2021, 4, 3, 9, 30), 22)).toEqual(
  ...
  describe('getStartOfWeekEnd', () => {
    it.each`
      ...
      ${sunday}       | ${sunday}
    `(
      'startOfWeekEnd of $currentDate should be "$expected"',
      ({ currentDate, expected }: { currentDate: Date; expected: Date }) => {
        expect(TIME.getStartOfWeekEnd(currentDate)).toEqual(TIME.getStartOfDay(expected))
      }
    )
  })
  ...
  describe('getEndOfWeek', () => {
    it.each`
      ...
      ${sunday}       | ${sunday}
    `(
      'endOfWeek of $currentDate should be "$expected"',
      ({ currentDate, expected }: { currentDate: Date; expected: Date }) => {
        expect(TIME.getEndOfWeek(currentDate)).toEqual(TIME.getEndOfDay(expected))
      }
    )
  })
  ...
  describe('getWeekEndDatesFromDate', () => {
    it.each`
      ...
      ${sunday}       | ${[sunday]}
    `(
      'should get week end dates of $currentDate',
      ({ currentDate, expected }: { currentDate: Date; expected: Date[] }) => {
  ...
  describe('getWeekDatesFromDate', () => {
    it.each`
      ...
      ${sunday}    | ${[sunday]}
    `(
      'should get week dates of $currentDate',
      ({ currentDate, expected }: { currentDate: Date; expected: Date[] }) => {
  ...
  describe('getAllFromTimeRangeAndDate', () => {
    const timeRange: Range<number> = [20, 23]
