...
describe('useFormatFullAddress', () => {
  it.each`
    publicName              | name            | address               | postalCode   | city         | addressWithoutVenueName | expectedResult
    ...
    ${'Pathé beaugrenelle'} | ${'Ciné Pathé'} | ${'2 rue des champs'} | ${''}        | ${'Paris'}   | ${false}                | ${'Pathé beaugrenelle, 2 rue des champs, Paris'}
  `(
    'should format correctly full address with venue name',
    ({ publicName, name, address, postalCode, city, addressWithoutVenueName, expectedResult }) => {
      expect(
        formatFullAddress(publicName, name, address, postalCode, city, addressWithoutVenueName)
  ...
  `(
    'should format correctly full address without venue name',
    ({ publicName, name, address, postalCode, city, addressWithoutVenueName, expectedResult }) => {
      expect(
        formatFullAddress(publicName, name, address, postalCode, city, addressWithoutVenueName)
      ).toEqual(expectedResult)
