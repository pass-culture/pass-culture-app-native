...
export async function handleGeneratedApiResponse<T>(response: Response): Promise<T | EmptyObject> {
  ...
  const responseBody: T | string = contentType?.includes('application/json')
    ? await response.json()
    : await response.text()
