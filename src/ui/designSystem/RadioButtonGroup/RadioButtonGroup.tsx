...
import React, { FunctionComponent, useCallback, SetStateAction, useMemo } from 'react'
import { FlatList, FlatListProps, ListRenderItemInfo, Platform } from 'react-native'
...
const generateRadioAccessibilityLabel = (
  ...
  return getComputedAccessibilityLabel(
    ...
    item.label,
    item.description,
    Platform.OS === 'web' ? undefined : selectionLabel
  )
}
...
export const RadioButtonGroup: FunctionComponent<Props> = ({
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      const currentIndex = enabledOptions.findIndex((opt) => opt.label === value)
      const step = isNext ? 1 : -1

      const nextIndex = (currentIndex + step + enabledOptions.length) % enabledOptions.length
      const targetOption = enabledOptions[nextIndex]

      if (targetOption) {
        onChange(targetOption.label)
      }
    },
  ...
  const renderRadioButton = useCallback(
    ...
    [value, disabled, activeIndex, error, handleSetValue, variant, display, label, totalOptions]
  )
