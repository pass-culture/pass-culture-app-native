...
describe('PhysicalGoodBookingTicket', () => {
  it('should not display anything if voucher is valid but token and ean are null', async () => {
  ...
  it('should not display anything if token is valid but voucher and ean are null', async () => {
    renderPhysicalGoodBookingTicket({ voucher: null, token: { data: 'test-token' }, ean: null })

    expect(screen.queryByTestId('physical-good-booking-ticket-container')).not.toBeOnTheScreen()
  })

  it('should not display anything if ean is valid but token and voucher are null', async () => {
    renderPhysicalGoodBookingTicket({ voucher: null, token: { data: 'test-token' }, ean: '1234' })

    expect(screen.queryByTestId('physical-good-booking-ticket-container')).not.toBeOnTheScreen()
  })

  it('should display container if token, voucher and ean are present', async () => {
