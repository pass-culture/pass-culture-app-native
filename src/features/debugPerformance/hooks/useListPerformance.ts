...
import { ListMonitor, ListMonitorOptions, ListRef, listMonitor } from '../services/ListMonitor'
...
export interface UseListPerformanceReturn {
  ...
  getRecentRenderEvents: (limit?: number) => any[]
...
export const useListPerformance = <T>(
  ...
  // FlashList reference
  const flashListRef = listType === 'FlashList' ? (listRef as React.RefObject<FlashListRef<T>>) : null

  const startMonitoring = useCallback((): string | null => {
    return safeExecute(
      () => {
        logger.verbose('🚀 [LIST PERFORMANCE] Starting monitoring - isRecording:', isRecording, 'sessionId:', sessionId)
    ...
    safeExecute(
      () => {
        ...
        if (lastScrollTime.current > 0) {
          ...
          const distanceDiff = Math.abs(scrollTop - (event.nativeEvent as any).previousScrollTop || 0)
        ...
        const monitor = (listMonitor as any).activeMonitors?.get(listIdRef.current)
        if (monitor) {
  ...
  const runBenchmark = useCallback(async (): Promise<any | null> => {
    return safeExecute(
      () => {
