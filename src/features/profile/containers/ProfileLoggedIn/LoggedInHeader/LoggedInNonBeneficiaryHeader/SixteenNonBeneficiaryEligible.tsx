...
export const SixteenNonBeneficiaryEligible = ({ featureFlags, user }: Props) => {
  const { firstName, lastName, birthDate, eligibility } = user
  const title = getProfileHeaderTitle({ firstName, lastName })
  const age = getAge(birthDate) ?? 16

  return (
    <ViewGap gap={6} testID="sixteen-non-beneficiary-eligible-header">
