...
import { checkInstalledApps } from 'features/offer/helpers/checkInstalledApps'
    ...
    <React.Fragment>
      {installedApps.map((network) => {
        const {
          ...
          isNative,
          ...options
        } = mapNetworkToSocial[network as Network]
        ...
        return (
          <TouchableOpacity key={network} onPress={onPress}>
            <ShareMessagingApp network={network as Network} />
          </TouchableOpacity>
        )
      })}
    </React.Fragment>
