...
export function OfferPlaylistList({
  ...
  const onViewableFor = useCallback(
    (playlistId: string) => (items: Pick<ViewToken, 'key' | 'index'>[]) => {
      if (!isFocused) return
      onPlaylistViewableItemsChanged?.(playlistId, items)
    },
    [isFocused, onPlaylistViewableItemsChanged]
  )
    ...
    <SectionWithDivider visible={shouldDisplayPlaylist} gap={8}>
      {similarOffersPlaylist.map((playlist) => {
        ...
        return (
          <ObservedPlaylist
            key={playlist.type}
            onViewableItemsChanged={onViewableFor(playlist.type)}>
