...
export const getCtaWordingAndAction = ({
  ...
  const isBookOffer = offer?.subcategoryId === SubcategoryIdEnum.LIVRE_PAPIER
  ...
  const enableLoanFakedoor = featureFlags.wipEnableLoanFakeDoor
  ...
  if (isBookOffer && enableLoanFakedoor) {
    return [
      {
        modalToDisplay: OfferModal.BOOKING,
        wording: 'acheter',
