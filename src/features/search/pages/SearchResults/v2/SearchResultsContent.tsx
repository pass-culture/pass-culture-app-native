...
import { SearchResultsListContent } from 'features/search/pages/SearchResults/v2/components/SearchResultsListContent'
...
export const SearchResultsContent: FC<Props> = ({
  ...
  const shouldDisplayArtistSection = !selectedFilter && searchListContent?.rawHits?.artists?.length
    ...
    <React.Fragment>
      ...
      <Container testID="searchResults">
        <FlashList
          ...
          data={searchListContent.listItems}
          keyExtractor={keyExtractor}
          getItemType={(item: SearchResultItem) => item.type}
          ListHeaderComponent={
            <SearchResultsListHeader
              nbHits={searchListContent.listNbHits}
              title={searchListContent.listTitle}
              userData={userData}
              venuesSection={
                shouldDisplayVenueSection ? (
                  <VenueSection
                    venues={searchListContent.rawHits?.venues}
                    venuesUserData={venuesUserData}
              ...
              artistSection={
                shouldDisplayArtistSection ? (
                  <StyledArtistSection
                    artists={searchListContent.rawHits?.artists}
                    onArtistPlaylistItemPress={handleOnArtistPlaylistItemPress}
                    withMargins={false}
                  />
                ) : undefined
