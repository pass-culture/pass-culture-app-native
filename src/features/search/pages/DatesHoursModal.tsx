...
export const DatesHoursModal: FunctionComponent<Props> = ({
  ...
  const logToggleDate = useLogFilterOnce(SectionTitle.Date)
  const logToggleHour = useLogFilterOnce(SectionTitle.Hour)
  const logSelectedDateChoice = useLogFilterOnce(SectionTitle.OfferDate)
  const logChangeSelectedHours = useLogFilterOnce(SectionTitle.TimeSlot)
