  ...
  const handleGtlViewableItemsChanged = useCallback(
    (playlistTitle: string, playlistIndex: number) =>
      (items: Pick<ViewToken, 'key' | 'index'>[]) => {
        console.log('handleGtlViewableItemsChanged', { playlistTitle, items, isFocused })
        if (!isFocused) return
          ...
          <ObservedPlaylist
            ...
            {({ listRef, handleViewableItemsChanged }) => (
              <GtlPlaylist
                ...
                noMarginBottom
                playlistRef={listRef}
                onViewableItemsChanged={(info: { viewableItems: ViewToken<unknown>[] }) => {
                  console.log('GtlPlaylist onViewableItemsChanged', { info })
