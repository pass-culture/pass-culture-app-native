...
describe('price schema', () => {
  describe('should match', () => {
    it('a valid integer price', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate('123')
      expect(result).toEqual('123')
    })

    it('a valid price with a dot', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate('123.45')
    ...
    it('a valid price with only one decimal', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate('123,4')
    ...
    it('when input less than the initial credit', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate('200')
      expect(result).toEqual('200')
    })

    it('when input equal to the initial credit', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate(MAX_PRICE.toString())
      expect(result).toEqual(MAX_PRICE.toString())
    })

    it('when input is undefined', async () => {
      const result = await priceSchema(MAX_PRICE.toString()).validate(undefined)
  ...
  describe('should invalidate an input', () => {
    ...
    it('that is not a number', async () => {
      const result = priceSchema(MAX_PRICE.toString()).validate('azerty')
    ...
    it('that is a number with more than 2 decimals', async () => {
      const result = priceSchema(MAX_PRICE.toString()).validate('123.456')

      await expect(result).rejects.toEqual(new ValidationError(formatErrorMessage))
    })

    it('that is a number with more than 2 comma or dot', async () => {
      const result = priceSchema(MAX_PRICE.toString()).validate('1,23.45')

      await expect(result).rejects.toEqual(new ValidationError(formatErrorMessage))
    })

    it('that is a number with negative number', async () => {
      const result = priceSchema(MAX_PRICE.toString()).validate('-123.45')
    ...
    it('when input higher than the initial credit', async () => {
      const result = priceSchema(MAX_PRICE.toString()).validate('400')
