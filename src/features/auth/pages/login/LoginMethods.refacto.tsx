...
import { LoginRoutineMethod } from 'libs/analytics/logEventAnalytics'
...
export const LoginMethods = () => {
  ...
  const handleLoginSuccess = async (
    ...
    response: AxiosResponse<SigninResponseV2>
  ) => {
    void analytics.logLogin({ method, type: 'SSO_login' })
  ...
  const { mutate: signInGoogle } = useSSOLoginMutation({
    onFailure: handleLoginFailure,
    onSuccess: (response: AxiosResponse<SigninResponseV2>) =>
      handleLoginSuccess('fromLoginGoogle', response),
  })

  const { mutate: signInApple } = useSSOLoginMutation({
    onFailure: handleLoginFailure,
    onSuccess: (response: AxiosResponse<SigninResponseV2>) =>
      handleLoginSuccess('fromLoginApple', response),
  })
